/*  
   Name        : Rohan Sagar Belekar
   Date        : 18/07/2026
   Project     : Code-to-HTML Converter
   Objective   : Convert given C source code into an HTML file 
                 with syntax highlighting for easier readability.
   Description : The converter parses the input C program and 
                 generates an HTML output. Reserved keywords, 
                 preprocessor directives, numerical constants, 
                 strings, and comments are wrapped in <span> tags 
                 with predefined CSS classes for color styling. 
                 The output file can be opened in any browser for 
                 a clear, formatted code walkthrough.
   Scope       : Supports single C source files. Highlighting 
                 depends on external CSS definitions. 
*/
